...
void RefreshCustomRESTResponseSwamp(char *IPWAN, char *IPLAN, uint8_t pumpState, uint8_t fanState, DHT22_Data *tempAndHumid, uint32_t dht_errors)
{
#ifndef NODE_ID
#error NODE_ID not defined, Please define NODE_ID as char*
#endif
snprintf(customRESTResponse, ARRAYSIZE(customRESTResponse),"{\"ID\":\"%s\",\"NodeStatus\":{\"pumpState\":\"%d\",\"fanState\":\"%d\",\"currentTemp\":\"%.2f\",\"currentHumid\":\"%.2f\"},\"GeneralStatus\":{\"CurrentIP_WAN\":%s,\"currentIP_LAN\":%s,\"dht_errors\":%d,\"self_check_result\":\"OK\"}} ",NODE_ID, pumpState, fanState, tempAndHumid->Temp, tempAndHumid->Humid, IPWAN, IPLAN, dht_errors);
}
