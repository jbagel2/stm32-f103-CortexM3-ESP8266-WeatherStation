...
IPD_Data Wifi_CheckDMABuff_ForIPDData(DHT22_Data *Current_DHT22_Reading)
{
	...
	if(ESP_IPD_Data_Buffer_Pntr)
	{
			...
			if(multiQuery)
			{
				...
				for(qsc; qsc < 4; qsc++)
				{
					...
					if(thisQuery.key=="\0" || thisQuery.key == NULL)
					{
						...
						{
							currentIPD.Valid = 1;
							RefreshCustomRESTResponseSwamp(currentESPStatus.Station_IP, currentESPStatus.AccessPoint_IP, 0, 0,Current_DHT22_Reading, DHT_Error_Since_Boot);
						}
						return currentIPD;
					}
					else {
		...
		else if (requestType == GET)
		{
			...
			//TODO: Need to add IPD requesat breakdown for more detailed query for get
			RefreshCustomRESTResponseSwamp(currentESPStatus.Station_IP, currentESPStatus.AccessPoint_IP, 0, 0,Current_DHT22_Reading, DHT_Error_Since_Boot);
			currentIPD.Valid = 1;
			return currentIPD;
		}
